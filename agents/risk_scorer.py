...
class RiskScorer:
    ...
    def calculate_final_score(self, company: str, all_analyses: Dict[str, Any]) -> Dict[str, Any]:
        ...
        final_score = max(0, min(100, round(base_risk_score + industry_adjustment + peer_modifier, 1)))
        ...
        if override_ml:
            ...
            final_score = round(
                min(100, max(0, final_score + industry_adjustment)), 1
            )
        ...
        try:
            ...
            if override_ml:
                ...
                final_score = round(
                    min(100, max(0, blended_score_synced + industry_adjustment)), 1
                )
            else:
                final_score = round(
                    min(100, max(0, max(pillar_greenwashing_synced, base_risk_score) + industry_adjustment)), 1
                )
